...
Copy the old Hive's folder anywhere under your Hives folder, then ask your Brainstem to bring its pending requests along.
The `import` action:

1. reads the old folder's identity records and RAPP/1 frames;
...
Nobody is admitted by an import. Members admit carried requests exactly like new ones.
...
2. Signed join requests that are still waiting are carried byte for byte with `import`, after the old Hive's id is listed
   in `previous`.
